 ...
 */
 ...
 bool GpuStark::check_gpu_memory(size_t padded_height) {
     ...
     const size_t THRESHOLD_HEIGHT = 2097152/2;  // 2^21
 ...
 void GpuStark::step_initialize_fiat_shamir(const Claim& claim) {
     ...
     // NOTE: Log2PaddedHeight IS included in Fiat-Shamir (updated in xnt-core patch).
     // Must enqueue into fs_cpu_ for Fiat-Shamir absorption
     fs_cpu_.enqueue(ProofItem::make_log2_padded_height(log2_height));
 ...
 void GpuStark::step_fri_protocol() {
     ...
     // BEFORE sampling query indices. We use the CPU transcript for gold-standard matching.
     {
         ...
         // Compute FriPolynomial using Rust FFI (exact interpolation behavior)
         uint64_t* poly_ptr = nullptr;
         size_t poly_len = 0;
         int rc = fri_interpolate_last_polynomial_rust(
             h_last_codeword.data(),
             current_size,
             &poly_ptr,
             &poly_len
         ...
         if (rc != 0) {
             throw std::runtime_error("fri_interpolate_last_polynomial_rust failed in step_fri_protocol");
         }

         // Build polynomial coefficients
         std::vector<XFieldElement> poly_coeffs;
         poly_coeffs.reserve(poly_len / 3);
         for (size_t i = 0; i + 2 < poly_len; i += 3) {
             poly_coeffs.emplace_back(
                 BFieldElement(poly_ptr[i + 0]),
                 BFieldElement(poly_ptr[i + 1]),
                 BFieldElement(poly_ptr[i + 2])
             );
         }
 ...
 void GpuStark::step_fri_protocol_frugal() {
     ...
     // BEFORE sampling query indices. We use the CPU transcript for gold-standard matching.
     {
         ...
         if (rc != 0) {
             throw std::runtime_error("fri_interpolate_last_polynomial_rust failed in step_fri_protocol_frugal");
         }
