 ...
 */
 ...
 // Note: Texture memory removed - using __ldg for cached reads instead
 ...
 __global__ void batched_ntt_butterfly_ilp8_kernel(
     ...
     for (int k = 0; k < ILP; k++) {
         ...
         // OPTIMIZED: Use __ldg for cached read-only access
         const uint64_t tw = __ldg(&d_twiddles[twiddle_offset + pos]);
 ...
 __global__ void batched_ntt_fused_first10_kernel(
     ...
     for (int stage = 0; stage < FUSED_STAGES; stage++) {
         ...
         for (int b = 0; b < butterflies_per_thread; b++) {
             ...
             // OPTIMIZED: Use __ldg for cached read-only access
             const uint64_t twiddle = __ldg(&d_twiddles[twiddle_offset + pos]);
 ...
 __global__ void batched_intt_fused_first10_kernel(
     ...
     for (int stage = 0; stage < FUSED_STAGES; stage++) {
         ...
         for (int b = 0; b < butterflies_per_thread; b++) {
             ...
             // OPTIMIZED: Use __ldg for cached read-only access
             const uint64_t twiddle = __ldg(&d_twiddles_inv[twiddle_offset + pos]);
