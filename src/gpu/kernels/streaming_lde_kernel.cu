...
// Helper functions for Goldilocks field arithmetic (device and host)
namespace {

constexpr uint64_t GOLDILOCKS_P = 0xFFFFFFFF00000001ULL;

// Montgomery reduction helper (device version)
__device__ __forceinline__ uint64_t mul_mod_device(uint64_t a, uint64_t b) {
    // Use CUDA intrinsics for 64-bit multiplication
    uint64_t lo = a * b;
    uint64_t hi = __umul64hi(a, b);
    
    // Barrett-like reduction for Goldilocks
    uint64_t q = hi;
    uint64_t r = lo - q * GOLDILOCKS_P;
    if (r >= GOLDILOCKS_P) r -= GOLDILOCKS_P;
    return r;
}

__device__ uint64_t bfield_pow_device(uint64_t base, uint64_t exp) {
    uint64_t result = 1;
    base %= GOLDILOCKS_P;
    while (exp > 0) {
        if (exp & 1) {
            result = mul_mod_device(result, base);
        }
        exp >>= 1;
        base = mul_mod_device(base, base);
    }
    return result;
}

// Host version for host wrapper functions
inline uint64_t mul_mod_host(uint64_t a, uint64_t b) {
...
inline uint64_t bfield_pow_host(uint64_t base, uint64_t exp) {
    ...
    while (exp > 0) {
        if (exp & 1) {
            result = mul_mod_host(result, base);
        }
        exp >>= 1;
        base = mul_mod_host(base, base);
    }
    return result;
}
...
__global__ void evaluate_on_coset_bfield_kernel(
    ...
    // Get working domain generator (primitive root of unity of order working_domain_len)
    // This should be precomputed and passed in, but for simplicity compute here
    // omega_working = omega_trace^(trace_len / working_domain_len) = omega_trace^RATIO
    
    // Actually, the evaluation point is: coset_offset × omega_working^idx
    // But polynomial has trace_len coefficients, so we need to zero-pad to working_domain_len
    // OR use the fact that working_domain_len ≤ trace_len
    ...
    if (num_randomizers > 0 && d_randomizer_coeffs != nullptr) {
        ...
        for (size_t j = 0; j < num_randomizers; ++j) {
            ...
            rand_sum = add_mod(rand_sum, d_randomizer_coeffs[col * num_randomizers + j]);
        }
        sum = add_mod(sum, mul_mod(zerofier_value, rand_sum));
    }
...
__global__ void accumulate_coset_quotient_kernel(
    ...
    uint64_t scale = bfield_pow_device(iota, coset_index * idx);
    ...
    uint64_t s0 = mul_mod(q0, scale);
    uint64_t s1 = mul_mod(q1, scale);
    uint64_t s2 = mul_mod(q2, scale);
