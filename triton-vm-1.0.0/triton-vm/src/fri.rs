...
pub(crate) struct FriDebugData {
    pub merkle_roots: Vec<Digest>,
    pub folding_challenges: Vec<XFieldElement>,
    pub folded_codewords: Vec<Vec<XFieldElement>>,  // Codewords after each fold
    pub last_codeword: Option<Vec<XFieldElement>>,
    pub last_polynomial: Option<Vec<XFieldElement>>,
    pub sponge_state_before_query: Option<[BFieldElement; 16]>,  // Sponge state before sampling indices (Tip5 STATE_SIZE)
}
...
struct FriProver<'stream> {
    ...
    debug_data: Option<&'stream mut FriDebugData>,
}
...
impl FriProver<'_> {
    ...
    fn commit_to_round(&mut self, round: &ProverRound) {
        ...
        if let Some(ref mut debug_data) = self.debug_data {
            debug_data.merkle_roots.push(merkle_root);
        }
    ...
    fn construct_next_round(&mut self) -> ProverResult<ProverRound> {
        ...
        if let Some(ref mut debug_data) = self.debug_data {
            debug_data.folding_challenges.push(folding_challenge);
        }
        let codeword = previous_round.split_and_fold(folding_challenge);
        if let Some(ref mut debug_data) = self.debug_data {
            debug_data.folded_codewords.push(codeword.clone());
        }
    ...
    fn send_last_codeword(&mut self) {
        ...
        if let Some(ref mut debug_data) = self.debug_data {
            debug_data.last_codeword = Some(last_codeword.clone());
        }
    ...
    fn send_last_polynomial(&mut self) {
        ...
        if let Some(ref mut debug_data) = self.debug_data {
            // Convert polynomial to vector of coefficients for dumping
            let polynomial_coeffs: Vec<XFieldElement> = last_polynomial.coefficients().to_vec();
            debug_data.last_polynomial = Some(polynomial_coeffs);
        }
    ...
    fn sample_first_round_collinearity_check_indices(&mut self) {
        // Capture sponge state right before sampling indices (for C++ verification)
        if let Some(ref mut debug_data) = self.debug_data {
            debug_data.sponge_state_before_query = Some(self.proof_stream.sponge.state);
        }
...
impl Fri {
    ...
    pub fn prove(
        ...
        self.prove_with_debug(codeword, proof_stream, None)
    }

    /// Create a FRI proof with optional debug data collection.
    pub(crate) fn prove_with_debug(
        &self,
        codeword: &[XFieldElement],
        proof_stream: &mut ProofStream,
        debug_data: Option<&mut FriDebugData>,
    ) -> ProverResult<Vec<usize>> {
        let mut prover = self.prover(proof_stream, debug_data);
