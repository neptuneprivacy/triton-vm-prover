...
impl MasterMainTable {
    ...
    /// Get the domains for this table.
    pub fn domains(&self) -> ProverDomains {
        MasterTable::domains(self)
    }

    /// Get the trace table view.
    pub fn trace_table(&self) -> ArrayView2<'_, BFieldElement> {
        MasterTable::trace_table(self)
    }

    /// Get the quotient domain table if available.
    pub fn quotient_domain_table(&self) -> Option<ArrayView2<'_, BFieldElement>> {
        MasterTable::quotient_domain_table(self)
    }

    /// Get the trace randomizer for a column.
    pub fn trace_randomizer_for_column(&self, idx: usize) -> Polynomial<'static, BFieldElement> {
        MasterTable::trace_randomizer_for_column(self, idx)
    }

    /// Get the randomized column interpolant.
    pub fn randomized_column_interpolant(&self, idx: usize) -> Polynomial<'static, BFieldElement> {
        MasterTable::randomized_column_interpolant(self, idx)
    }

    /// Get the trace randomizer seed.
    pub fn trace_randomizer_seed(&self) -> <StdRng as SeedableRng>::Seed {
        MasterTable::trace_randomizer_seed(self)
    }

    /// Get the number of trace randomizers.
    pub fn num_trace_randomizers(&self) -> usize {
        MasterTable::num_trace_randomizers(self)
    }

    /// Low-degree extend all columns if not already done.
    pub fn maybe_low_degree_extend_all_columns(&mut self) {
        MasterTable::maybe_low_degree_extend_all_columns(self);
    }

    /// Compute a Merkle tree of the FRI domain table.
    pub fn merkle_tree(&self) -> MerkleTree {
        MasterTable::merkle_tree(self)
    }
