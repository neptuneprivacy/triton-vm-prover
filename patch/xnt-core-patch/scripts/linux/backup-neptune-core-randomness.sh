...
# 0 3 * * * ~/bin/backup-xnt-core-randomness.sh
...
rsync -av --backup --suffix=_$(date +'%Y-%m-%d_%H-%M-%S') \
      --backup-dir=~/xnt-core-mainnet-randomness-backup/<machine-id>/$(date +'%Y-%m-%d') \
      ~/.local/share/neptune/main/wallet/incoming_randomness.dat ~/.local/share/neptune/main/wallet/outgoing_randomness.dat <user>@<remote-server>:~/xnt-core-mainnet-randomness-backup/<machine-id>/
