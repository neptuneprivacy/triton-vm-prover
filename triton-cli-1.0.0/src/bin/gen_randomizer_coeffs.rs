...
fn main() -> Result<()> {
    ...
    let master_main_table = MasterMainTable::new(&aet, domains, stark.num_trace_randomizers, seed);
    ...
    let randomizer_coeffs: Vec<u64> = trace_randomizer
        .coefficients()
        .iter()
        .map(|x| x.value())
        .collect();
    ...
    let seed_hex: String = trace_randomizer_seed
        .iter()
        .map(|&b| format!("{:02x}", b))
        .collect();
    ...
    fs::write(
        output_dir.join("04_randomizer_coefficients.json"),
    ...
    println!(
        "     Column interpolant coefficients: {}",
        column_interpolant.coefficients().len()
    );
