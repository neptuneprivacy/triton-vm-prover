...
use triton_vm::stark::{Prover, ProverDomains, Stark};
...
fn main() -> Result<()> {
    ...
    if args.len() < 3 {
        eprintln!(
            "Usage: {} <program.tasm> <public_input> [output_dir]",
            args[0]
        );
        eprintln!(
            "Example: {} spin_input8.tasm 8 /tmp/rust_debug_quotient",
            args[0]
        );
        std::process::exit(1);
    }
    ...
    // Parse public input
    let public_input_value: u64 = public_input_str
        .parse()
    ...
    let mut main_table = MasterMainTable::new(
        &aet,
        domains.clone(),
        stark.num_trace_randomizers,
        [0u8; 32],
    ...
    zf_json.insert(
        "quotient_len".to_string(),
        serde_json::Value::Number(domains.quotient.length.into()),
    );
    zf_json.insert(
        "trace_len".to_string(),
        serde_json::Value::Number(domains.trace.length.into()),
    );
    ...
    qc_json.insert(
        "quotient_len".to_string(),
        serde_json::Value::Number(quotient_codeword.len().into()),
    );
    let qv_array: Vec<serde_json::Value> = quotient_codeword
        .iter()
        .take(20)
        .map(|xfe| {
            ...
            obj.insert(
                "c0".to_string(),
                serde_json::Value::Number(xfe.coefficients[0].value().into()),
            );
            obj.insert(
                "c1".to_string(),
                serde_json::Value::Number(xfe.coefficients[1].value().into()),
            );
            obj.insert(
                "c2".to_string(),
                serde_json::Value::Number(xfe.coefficients[2].value().into()),
            );
            serde_json::Value::Object(obj)
        })
        .collect();
    qc_json.insert(
        "quotient_values".to_string(),
        serde_json::Value::Array(qv_array),
    );
    ...
    let initial_constraints =
        MasterAuxTable::evaluate_initial_constraints(first_row_main, first_row_aux, &challenges);
    ...
    let terminal_constraints =
        MasterAuxTable::evaluate_terminal_constraints(first_row_main, first_row_aux, &challenges);
    ...
    let initial_array: Vec<serde_json::Value> =
        initial_constraints.iter().map(xfe_to_json).collect();
    let consistency_array: Vec<serde_json::Value> =
        consistency_constraints.iter().map(xfe_to_json).collect();
    let transition_array: Vec<serde_json::Value> =
        transition_constraints.iter().map(xfe_to_json).collect();
    let terminal_array: Vec<serde_json::Value> =
        terminal_constraints.iter().map(xfe_to_json).collect();

    cr_json.insert(
        "initial_constraints".to_string(),
        serde_json::Value::Array(initial_array),
    );
    cr_json.insert(
        "consistency_constraints".to_string(),
        serde_json::Value::Array(consistency_array),
    );
    cr_json.insert(
        "transition_constraints".to_string(),
        serde_json::Value::Array(transition_array),
    );
    cr_json.insert(
        "terminal_constraints".to_string(),
        serde_json::Value::Array(terminal_array),
    );
    cr_json.insert(
        "quotient_value".to_string(),
        xfe_to_json(&quotient_codeword[0]),
    );
    ...
    wc_json.insert(
        "num_weights".to_string(),
        serde_json::Value::Number(quotient_weights.len().into()),
    );
    let weights_array: Vec<serde_json::Value> =
        quotient_weights.iter().take(20).map(xfe_to_json).collect();
    wc_json.insert(
        "weights".to_string(),
    ...
    wc_json.insert(
        "num_challenges".to_string(),
        serde_json::Value::Number(Challenges::SAMPLE_COUNT.into()),
    );
    let challenges_array: Vec<serde_json::Value> = challenges
        .challenges
        .iter()
        .take(10)
        .map(xfe_to_json)
    ...
    wc_json.insert(
        ...
        serde_json::Value::Array(challenges_array),
    );
    ...
    pc_json.insert(
        "num_coeffs".to_string(),
        serde_json::Value::Number(poly_coeffs.len().into()),
    );
    let coeffs_array: Vec<serde_json::Value> =
        poly_coeffs.iter().take(50).map(xfe_to_json).collect();
    ...
    let segments_array: Vec<serde_json::Value> = segments
        .iter()
        .enumerate()
        .map(|(idx, seg)| {
            ...
            seg_obj.insert(
                "num_coeffs".to_string(),
                serde_json::Value::Number(coeffs.len().into()),
            );
            let coeffs_array: Vec<serde_json::Value> =
                coeffs.iter().take(20).map(xfe_to_json).collect();
    ...
    sp_json.insert(
        "segments".to_string(),
        serde_json::Value::Array(segments_array),
    );
    ...
    let sum_of_segments: XFieldElement = ood_evaluations
        .iter()
    ...
    let seg_evals_array: Vec<serde_json::Value> = ood_evaluations
        .iter()
        .enumerate()
        .map(|(idx, (eval, power))| {
            let mut seg_obj = serde_json::Map::new();
        ...
        .collect();
    ood_json.insert(
        "segment_evaluations".to_string(),
        serde_json::Value::Array(seg_evals_array),
    );
