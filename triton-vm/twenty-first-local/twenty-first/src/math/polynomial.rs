...
use crate::math::traits::FieldKind;
    ...
    FF: FiniteField + MulAssign<BFieldElement> + FieldKind,
{
    ...
    where
        FF: Mul<FF2>,
        FF2: FiniteField + MulAssign<BFieldElement> + FieldKind,
        <FF as Mul<FF2>>::Output: FiniteField + MulAssign<BFieldElement> + FieldKind,
    {
        ...
        FF: Mul<S, Output = FF> + 'static,
    {
        ...
        // Fuse scale + resize into single allocation for better performance
        // Pre-allocate and initialize with zeros
        let mut coefficients = vec![FF::ZERO; order];
        let mut power_of_offset = S::one();

        // Scale coefficients directly into pre-allocated buffer
        for (i, &coeff) in self.coefficients.iter().enumerate() {
            coefficients[i] = coeff * power_of_offset.clone();
            power_of_offset = power_of_offset * offset.clone();
        }
