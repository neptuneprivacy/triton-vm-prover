...
#[cfg(feature = "gpu")]
use crate::table::gpu_quotient::GpuQuotientContext;
#[cfg(feature = "gpu")]
use std::sync::OnceLock;

#[cfg(feature = "gpu")]
static GPU_QUOTIENT_CONTEXT: OnceLock<GpuQuotientContext> = OnceLock::new();
...
use rand::prelude::*;
use strum::EnumCount;
use twenty_first::math::traits::FieldKind;
        ...
        + Add<XFieldElement, Output = XFieldElement>,
{
    ...
    fn maybe_low_degree_extend_all_columns(&mut self) {
        ...
        // Try GPU batched interpolation first
        #[cfg(feature = "gpu")]
        let batched_success = {
            use twenty_first::math::ntt::gpu_ntt::{get_gpu_batch_chunk_size, get_gpu_context};
            if get_gpu_context().is_some() {
                let chunk_size = get_gpu_batch_chunk_size();
                self.interpolate_columns_batched(
                    &column_indices,
                    &mut interpolation_polynomials,
                    chunk_size,
                )
                .is_ok()
            } else {
        ...
        #[cfg(not(feature = "gpu"))]
        let batched_success = false;

        if !batched_success {
            // Fallback to parallel CPU interpolation
        ...
        //skip this part because we do it on gpu
        /*
        extended_trace
            .spare_capacity_mut()
            .par_iter_mut()
        ...
        */
        ...
        // Keep row-major layout (default) for Merkle tree row hashing
        ...
        #[cfg(feature = "gpu")]
        let eval_batched_success = {
            use twenty_first::math::ntt::gpu_ntt::{get_gpu_batch_chunk_size, get_gpu_context};
            if get_gpu_context().is_some() {
                let chunk_size = get_gpu_batch_chunk_size();
                let result = self.evaluate_polynomials_batched(
                    &interpolation_polynomials,
                    &mut extended_columns,
                    evaluation_domain,
                    chunk_size,
                );
                match &result {
                    Ok(_) => eprintln!("[Evaluation] GPU batched evaluation succeeded"),
                    Err(e) => eprintln!(
                        "[Evaluation] GPU batched evaluation FAILED: {} - falling back to CPU",
                        e
                    ),
                }
                result.is_ok()
        ...
        #[cfg(not(feature = "gpu"))]
        let eval_batched_success = false;

        if !eval_batched_success {
            profiler!(start "CPU fallback evaluation");
            // Fallback to parallel CPU evaluation
            ...
            profiler!(stop "CPU fallback evaluation");
        }
        profiler!(stop "evaluation");
        // Note: Memoize even when GPU succeeds to provide fallback for edge cases
        // The GPU-extracted row cache takes priority in reveal_rows()
    ...
    /// Get cached GPU-computed row digests, if available (GPU feature only).
    #[cfg(feature = "gpu")]
    fn get_cached_row_digests(&self) -> Option<&Vec<Digest>>;

    /// Set cached GPU-computed row digests (GPU feature only).
    #[cfg(feature = "gpu")]
    fn set_cached_row_digests(&self, digests: Vec<Digest>);

    /// Get cached GPU-resident LDE buffer, if available (GPU feature only).
    /// This allows quotient evaluation to use GPU-resident data directly,
    /// avoiding wasteful GPU→RAM→GPU transfers.
    #[cfg(feature = "gpu")]
    fn get_gpu_lde_buffer(&self) -> Option<&crate::table::gpu_buffer::GpuLdeBuffer>;

    /// Set cached GPU-resident LDE buffer (GPU feature only).
    /// The buffer is kept alive via Arc and can be reused for quotient evaluation.
    #[cfg(feature = "gpu")]
    fn set_gpu_lde_buffer(&self, buffer: crate::table::gpu_buffer::GpuLdeBuffer);
    ...
    fn out_of_domain_row(&self, indeterminate: XFieldElement) -> Array1<XFieldElement> {
        ...
        let domain_over_domain_shift: Vec<XFieldElement> = domain
            .into_iter()
    ...
    /// GPU-batched interpolation for multiple columns
    /// Extracts column data, performs batched INTT via GPU, then adds randomizers
    #[cfg(feature = "gpu")]
    fn interpolate_columns_batched(
        &self,
        column_indices: &Array1<usize>,
        output_polynomials: &mut Array1<Polynomial<'static, Self::Field>>,
        _chunk_size: usize,
    ) -> Result<(), Box<dyn std::error::Error>>
    where
        Self::Field: MulAssign<BFieldElement> + FieldKind + 'static,
    {
        use twenty_first::math::ntt::{
            intt_bfield_fused_unscale_randomize, intt_xfield_fused_unscale_randomize,
        };

        let trace_table = self.trace_table();
        let trace_domain = self.domains().trace;
        let num_randomizers = self.num_trace_randomizers();

        eprintln!(
            "[GPU Batch Interpolation] Processing {} columns with fully fused INTT+unscale+randomize (ZERO-COPY)",
            column_indices.len()
        );

        // Pre-allocate column storage with space for randomizers (zero-copy approach)
        profiler!(start "prep: allocate & collect");
        use rayon::prelude::*;
        let mut column_data: Vec<Vec<Self::Field>> = column_indices
            .par_iter()
            .map(|&col_idx| {
                // Allocate with final size from the start
                let mut col = Vec::with_capacity(trace_domain.len() + num_randomizers);
                col.extend_from_slice(trace_table.column(col_idx).as_slice().unwrap());
                col
            })
            .collect();

        // Generate randomizers on CPU (small - only num_randomizers per column)
        let randomizers: Vec<Vec<Self::Field>> = column_indices
            .par_iter()
            .map(|&col_idx| {
                let mut rng = rng_from_offset_seed(self.trace_randomizer_seed(), col_idx);
                (0..num_randomizers).map(|_| rng.random()).collect()
            })
            .collect();

        // Compute offset^trace_length for zerofier multiplication
        let offset_power_n = trace_domain.offset().mod_pow(trace_domain.len() as u64);
        profiler!(stop "prep: allocate & collect");

        // Perform fully fused GPU operation (INTT + unscale + randomize)
        profiler!(start "GPU fused INTT+unscale+randomize" [gpu]);
        if Self::Field::IS_BASE {
            // BFieldElement path
            let mut column_data_bfield: Vec<Vec<BFieldElement>> =
                unsafe { std::mem::transmute(column_data) };
            let randomizers_bfield: Vec<Vec<BFieldElement>> =
                unsafe { std::mem::transmute(randomizers) };
            match intt_bfield_fused_unscale_randomize(
                &mut column_data_bfield,
                &randomizers_bfield,
                offset_power_n,
                "Trace Interpolation",
            ) {
                Err(e) => {
                    panic!("error at Trace Interpolation {:?}", e);
                }
                _ => {}
            }
            column_data = unsafe { std::mem::transmute(column_data_bfield) };
        } else {
            // XFieldElement path
            let mut column_data_xfield: Vec<Vec<XFieldElement>> =
                unsafe { std::mem::transmute(column_data) };
            let randomizers_xfield: Vec<Vec<XFieldElement>> =
                unsafe { std::mem::transmute(randomizers) };
            intt_xfield_fused_unscale_randomize(
                &mut column_data_xfield,
                &randomizers_xfield,
                offset_power_n,
                "Trace Interpolation",
            )?;
            column_data = unsafe { std::mem::transmute(column_data_xfield) };
        }
        profiler!(stop "GPU fused INTT+unscale+randomize");

        // Zero-copy: move Vec directly into Polynomial without cloning
        profiler!(start "wrap in Polynomial");
        for (&col_idx, data) in column_indices.iter().zip(column_data) {
            output_polynomials[col_idx] = Polynomial::new(data);
        }
        profiler!(stop "wrap in Polynomial");

        eprintln!(
            "[GPU Batch Interpolation] Successfully processed {} columns (ZERO-COPY)",
            column_indices.len()
        );
        Ok(())
    }

    /// GPU-batched evaluation for multiple polynomials
    /// Performs batched NTT via GPU on polynomial coefficients
    #[cfg(feature = "gpu")]
    fn evaluate_polynomials_batched(
        &self,
        polynomials: &Array1<Polynomial<'static, Self::Field>>,
        output_columns: &mut Array2<Self::Field>,
        evaluation_domain: ArithmeticDomain,
        chunk_size: usize,
    ) -> Result<(), Box<dyn std::error::Error>>
    where
        Self::Field: MulAssign<BFieldElement> + From<BFieldElement> + FieldKind + 'static,
    {
        // Note: Using unchunked API for better performance (no per-chunk serialization overhead)

        eprintln!(
            "[GPU Batch Evaluation] Processing {} polynomials (UNCHUNKED - single batch)",
            polynomials.len()
        );

        let eval_len = evaluation_domain.len();
        let offset = evaluation_domain.offset();

        // Write polynomial coefficients directly into output_columns
        // This avoids the separate allocation and later copy
        profiler!(start "GPU coset scale + prep" [gpu]);

        use ndarray::Axis;
        use rayon::prelude::*;

        // PERFORMANCE FIX: Write in row-major order to avoid cache thrashing!
        // The old column-wise approach caused 127-second delays due to strided writes.
        // Row-wise writing provides contiguous memory access for much better cache locality.

        /*
        let output_bfield = unsafe {
            std::slice::from_raw_parts_mut(
                output_columns.as_mut_ptr() as *mut BFieldElement,
                output_columns.len(),
            )
        };
        */

        let init_poly_result = if Self::Field::IS_BASE {
            let mut polys_coeff = Vec::new();
            for (_, poly) in polynomials.iter().enumerate() {
                let coeffs = poly.coefficients();
                let poly_bfield = unsafe {
                    std::slice::from_raw_parts(
                        coeffs.as_ptr() as *const BFieldElement,
                        coeffs.len(),
                    )
                };
                polys_coeff.push(poly_bfield);
            }

            let table_bfield = unsafe {
                std::slice::from_raw_parts_mut(
                    output_columns.as_mut_ptr() as *mut BFieldElement,
                    output_columns.len(),
                )
            ...
            let (nrows, ncols) = output_columns.dim();

            use twenty_first::math::ntt::gpu_ntt::get_gpu_context;
            if let Some(ctx) = get_gpu_context() {
                let result =
                    ctx.poly_fill_table_bfield(table_bfield, nrows, ncols, polys_coeff);
                result
            } else {
                panic!("GPU context not available for init polynomials bfield");
            }
        } else {
            let mut polys_coeff = Vec::new();
            for (_, poly) in polynomials.iter().enumerate() {
                let coeffs = poly.coefficients();
                let poly_xfield = unsafe {
                    std::slice::from_raw_parts(
                        coeffs.as_ptr() as *const XFieldElement,
                        coeffs.len(),
                    )
                };
                polys_coeff.push(poly_xfield);
            }

            let table_bfield = unsafe {
                std::slice::from_raw_parts_mut(
                    output_columns.as_mut_ptr() as *mut XFieldElement,
                    output_columns.len(),
                )
            };
            ...
            if let Some(ctx) = get_gpu_context() {
                let result =
                    ctx.poly_fill_table_xfield(table_bfield, nrows, ncols, polys_coeff);
                result
            } else {
                panic!("GPU context not available for init polynomials xfield");
            }
        };

        let init_poly_result = match init_poly_result {
            Ok(device_buffer)=>{
                device_buffer
            }
            Err(err)=>{
                panic!("Error initializing polynomials {:?}",err);
            }
        };

        
        /*
        Original cpu version, now moved to gpu
        ...
        output_columns
            ...
            .for_each(|(row_idx, mut row)| {
                for (col_idx, poly) in polynomials.iter().enumerate() {
                    let coeffs = poly.coefficients();
                    row[col_idx] = if row_idx < coeffs.len() {
                        coeffs[row_idx]
                    } else {
                        Self::Field::ZERO
                    };
        ...
        */
        ...
        profiler!(stop "GPU coset scale + prep");

        // Perform GPU fused coset scaling + NTT on the table directly
        profiler!(start "GPU coset scale + NTT" [multi-gpu]);

        profiler!(start "twiddle factors & setup");
        let len = eval_len;
        let log2_len = match len.checked_ilog2() {
            Some(log) => log,
            None => {
                profiler!(stop "twiddle factors & setup");
                profiler!(stop "GPU coset scale + NTT");
                return Err("Invalid array length".into());
            }
        };
        profiler!(stop "twiddle factors & setup");

        let gpu_result = if Self::Field::IS_BASE {
            // BFieldElement path: Use table-based GPU coset scaling + NTT
            use twenty_first::math::ntt::gpu_ntt::get_gpu_context;

            // Fused GPU coset scaling + NTT on entire table (row-major, strided access)
            if let Some(ctx) = get_gpu_context() {
                profiler!(start "get twiddle factors");
                use std::sync::OnceLock;
                ...
                let twiddle_factors = ALL_TWIDDLE_FACTORS[log2_len as usize].get_or_init(|| {
                    let omega = BFieldElement::primitive_root_of_unity(len as u64).unwrap();
                    twenty_first::math::ntt::twiddle_factors(len as u32, omega)
                });
                profiler!(stop "get twiddle factors");

                profiler!(start "cast table");
                // Cast output_columns to BFieldElement table
                let (nrows, ncols) = output_columns.dim();
                profiler!(stop "cast table");

                profiler!(start "GPU table operation" [multi-gpu]);
                let result = ctx.ntt_bfield_fused_coset_table(
                    init_poly_result,
                    nrows,
                    ncols,
                    offset,
                    twiddle_factors,
                    "BField Fused Coset+NTT",
                );
                profiler!(stop "GPU table operation");
                result
            } else {
                Err("GPU context not available for fused coset+NTT".into())
            }
        } else {
            // XFieldElement path: Use table-based GPU coset scaling + NTT
            use twenty_first::math::ntt::gpu_ntt::get_gpu_context;
            ...
            if let Some(ctx) = get_gpu_context() {
                ...
                use std::sync::OnceLock;
                const NUM_DOMAINS: usize = 32;
                static ALL_TWIDDLE_FACTORS_XFIELD: [OnceLock<Vec<Vec<BFieldElement>>>;
                    NUM_DOMAINS] = [const { OnceLock::new() }; NUM_DOMAINS];
                let twiddle_factors =
                    ALL_TWIDDLE_FACTORS_XFIELD[log2_len as usize].get_or_init(|| {
                        let omega = BFieldElement::primitive_root_of_unity(len as u64).unwrap();
                        twenty_first::math::ntt::twiddle_factors(len as u32, omega)
                    });
                profiler!(stop "get twiddle factors");
                ...
                profiler!(start "cast table");
                // Cast output_columns to XFieldElement table
                let (nrows, ncols) = output_columns.dim();
                profiler!(stop "cast table");

                profiler!(start "GPU table operation" [multi-gpu]);
                let result = ctx.ntt_xfield_fused_coset_table(
                    init_poly_result,
                    nrows,
                    ncols,
                    offset,
                    twiddle_factors,
                    "XField Fused Coset+NTT",
                );
                profiler!(stop "GPU table operation");
                result
            } else {
        ...
        // Check result after stopping profiler and cache digests & GPU buffer
        let (digests, gpu_buffer) = gpu_result?;

        // Cache GPU-computed digests for later use in hash_all_fri_domain_rows
        // AND cache GPU buffer to avoid wasteful GPU→RAM→GPU transfers in quotient evaluation
        #[cfg(feature = "gpu")]
        {
            self.set_cached_row_digests(digests);

            // Wrap GPU buffer with metadata and cache it
            use std::sync::Arc;
            let (nrows, ncols) = output_columns.dim();
            let element_size = if Self::Field::IS_BASE { 8 } else { 24 };
            let buffer_wrapper = crate::table::gpu_buffer::GpuLdeBuffer {
                device_buffer: Arc::new(gpu_buffer),
                num_rows: nrows,
                num_cols: ncols,
                element_size,
            };
            self.set_gpu_lde_buffer(buffer_wrapper);
            eprintln!(
                "[GPU-Resident LDE] Cached {:.2} GB GPU buffer (avoiding wasteful download)",
                (nrows * ncols * element_size) as f64 / 1_073_741_824.0
            );
        }

        // Note: output_columns still contains polynomial coefficients (not LDE results)
        // since we skipped the GPU→CPU download. We rely on partial row extraction
        // from GPU buffer later. No memoization needed since GPU path doesn't use CPU cache.

        eprintln!(
            "[GPU Batch Evaluation] Successfully processed {} polynomials",
            polynomials.len()
        );
        Ok(())
    }
    ...
    fn hash_all_fri_domain_rows(&self) -> Vec<Digest> {
        // Fast path: Use GPU-computed digests if available
        #[cfg(feature = "gpu")]
        if let Some(cached_digests) = self.get_cached_row_digests() {
            profiler!(start "hash rows" ("hash"));
            let digests = cached_digests.clone();
...
pub struct MasterMainTable {
    ...
    #[cfg(feature = "gpu")]
    cached_row_digests: std::sync::OnceLock<Vec<Digest>>,

    /// GPU-resident LDE table buffer (avoids wasteful GPU→RAM→GPU transfers)
    #[cfg(feature = "gpu")]
    gpu_lde_buffer: std::sync::OnceLock<crate::table::gpu_buffer::GpuLdeBuffer>,

    /// Cache for revealed rows (extracted from GPU for FRI proof)
    /// Maps row index to the revealed row data
    #[cfg(feature = "gpu")]
    pub(crate) revealed_rows_cache:
        std::sync::OnceLock<std::collections::HashMap<usize, Vec<BFieldElement>>>,
}
...
pub struct MasterAuxTable {
    ...
    #[cfg(feature = "gpu")]
    pub(crate) revealed_rows_cache:
        std::sync::OnceLock<std::collections::HashMap<usize, Vec<XFieldElement>>>,
}
...
impl MasterTable for MasterMainTable {
    ...
    #[cfg(feature = "gpu")]
    fn get_cached_row_digests(&self) -> Option<&Vec<Digest>> {
        self.cached_row_digests.get()
    }

    #[cfg(feature = "gpu")]
    fn set_cached_row_digests(&self, digests: Vec<Digest>) {
        let _ = self.cached_row_digests.set(digests);
    }

    #[cfg(feature = "gpu")]
    fn get_gpu_lde_buffer(&self) -> Option<&crate::table::gpu_buffer::GpuLdeBuffer> {
        self.gpu_lde_buffer.get()
    }

    #[cfg(feature = "gpu")]
    fn set_gpu_lde_buffer(&self, buffer: crate::table::gpu_buffer::GpuLdeBuffer) {
        let _ = self.gpu_lde_buffer.set(buffer);
    }
...
impl MasterMainTable {
    ...
    pub fn new(
        ...
        let mut master_main_table = Self {
            ...
            #[cfg(feature = "gpu")]
            cached_row_digests: std::sync::OnceLock::new(),
            #[cfg(feature = "gpu")]
            gpu_lde_buffer: std::sync::OnceLock::new(),
            #[cfg(feature = "gpu")]
            revealed_rows_cache: std::sync::OnceLock::new(),
        };

        // memory-like tables must be filled in before clock jump differences
        // are known, hence the break from the usual order

        profiler!(start "fill clk");
    
        profiler!(start "fill clk_jump_diffs_op_stack");
        let clk_jump_diffs_op_stack =
            OpStackTable::fill(master_main_table.table_mut(TableId::OpStack), aet, ());
        profiler!(stop "fill clk_jump_diffs_op_stack");

        profiler!(start "fill clk_jump_diffs_ram");
        let clk_jump_diffs_ram = RamTable::fill(master_main_table.table_mut(TableId::Ram), aet, ());
        profiler!(stop "fill clk_jump_diffs_ram");

        profiler!(start "fill clk_jump_diffs_jump_stack");
        let clk_jump_diffs_jump_stack =
            JumpStackTable::fill(master_main_table.table_mut(TableId::JumpStack), aet, ());
        profiler!(stop "fill clk_jump_diffs_jump_stack");
        ...
        profiler!(stop "fill clk");


        profiler!(start "fill ProcessorTable");
        let processor_table = master_main_table.table_mut(TableId::Processor);
        ProcessorTable::fill(processor_table, aet, clk_jump_diffs);
        profiler!(stop "fill ProcessorTable");

        /*

        profiler!(start "fill ProgramTable");
        ProgramTable::fill(master_main_table.table_mut(TableId::Program), aet, ());
        profiler!(stop "fill ProgramTable");

        profiler!(start "fill HashTable");
        HashTable::fill(master_main_table.table_mut(TableId::Hash), aet, ());
        profiler!(stop "fill HashTable");

        profiler!(start "fill CascadeTable");
        CascadeTable::fill(master_main_table.table_mut(TableId::Cascade), aet, ());
        profiler!(stop "fill CascadeTable");

        profiler!(start "fill LookupTable");
        LookupTable::fill(master_main_table.table_mut(TableId::Lookup), aet, ());
        profiler!(stop "fill LookupTable");

        profiler!(start "fill U32Table");
        U32Table::fill(master_main_table.table_mut(TableId::U32), aet, ());
        profiler!(stop "fill U32Table");
        */
        
// Parallelize the 5 independent table fills using horizontal_multi_slice_mut
        // to split the trace_table into non-overlapping mutable slices
        
        let all_tables = horizontal_multi_slice_mut(
            master_main_table.trace_table.view_mut(),
            &partial_sums(&[
        ...
        // Extract only the 5 independent tables: Program, Hash, Cascade, Lookup, U32
        // Indices: Program=0, Hash=5, Cascade=6, Lookup=7, U32=8
        let mut all_tables_iter = all_tables.into_iter();
        let program_table = all_tables_iter.next().unwrap();
        let _processor_table = all_tables_iter.next().unwrap();
        let _op_stack_table = all_tables_iter.next().unwrap();
        let _ram_table = all_tables_iter.next().unwrap();
        let _jump_stack_table = all_tables_iter.next().unwrap();
        let hash_table = all_tables_iter.next().unwrap();
        let cascade_table = all_tables_iter.next().unwrap();
        let lookup_table = all_tables_iter.next().unwrap();
        let u32_table = all_tables_iter.next().unwrap();

        let independent_tables = [program_table, hash_table, cascade_table, lookup_table, u32_table];
        let fill_functions = [
            ProgramTable::fill,
            HashTable::fill,
            CascadeTable::fill,
        ...
        fill_functions
            .into_par_iter()
            .zip_eq(independent_tables)
            .for_each(|(fill, table)| fill(table, aet, ()));
    ...
    /// Override reveal_rows to check GPU cache first
    #[cfg(feature = "gpu")]
    pub fn reveal_rows(&self, row_indices: &[usize]) -> Vec<Vec<BFieldElement>> {
        // Check GPU-extracted row cache first
        if let Some(cache) = self.revealed_rows_cache.get() {
            if row_indices.iter().all(|idx| cache.contains_key(idx)) {
                eprintln!(
                    "  [reveal_rows] Using GPU-extracted row cache ({} rows)",
                    row_indices.len()
                );
                return row_indices
                    .iter()
                    .map(|&row_idx| cache.get(&row_idx).unwrap().clone())
                    .collect();
            }
        }
        // Fall back to trait's reveal_rows
        <Self as MasterTable>::reveal_rows(self, row_indices)
    }
}
...
impl MasterAuxTable {
    ...
    /// Override reveal_rows to check GPU cache first
    #[cfg(feature = "gpu")]
    pub fn reveal_rows(&self, row_indices: &[usize]) -> Vec<Vec<XFieldElement>> {
        // Check GPU-extracted row cache first
        if let Some(cache) = self.revealed_rows_cache.get() {
            if row_indices.iter().all(|idx| cache.contains_key(idx)) {
                eprintln!(
...
pub fn all_quotients_combined(
    ...
    #[cfg(feature = "gpu")] gpu_main_buffer: Option<&crate::table::gpu_buffer::GpuLdeBuffer>,
    #[cfg(feature = "gpu")] gpu_aux_buffer: Option<&crate::table::gpu_buffer::GpuLdeBuffer>,
) -> Vec<XFieldElement> {
    assert_eq!(
        quotient_domain.len(),
    ...
    // Use GPU evaluation - PANIC if GPU fails (no CPU fallback)
    #[cfg(feature = "gpu")]
    {
        use std::time::Instant;
        
        let gpu_ctx = GPU_QUOTIENT_CONTEXT.get_or_init(|| {
            GpuQuotientContext::new()
                .expect("FATAL: Failed to initialize GPU quotient context. GPU feature is enabled but GPU is not available.")
        });

        let result = gpu_ctx
            .evaluate_quotients_gpu(
                ...
                challenges,
                quotient_weights,
                gpu_main_buffer.and_then(|buf| Some(buf.device_buffer())),
                gpu_aux_buffer.and_then(|buf| Some(buf.device_buffer())),
            )
            .expect(
                "FATAL: GPU quotient evaluation failed. No CPU fallback - this must work on GPU.",
            );

        eprintln!("  [GPU Quotient] Successfully evaluated on GPU");
        return result;
    }
