...
fn auxiliary_column_cascade_running_sum_log_derivative(
    ...
    #[cfg(feature = "gpu")]
    {
        gpu_cascade_running_sum_log_derivative(main_table, challenges)
            .expect("GPU cascade log derivative failed - GPU feature enabled but GPU unavailable or errored")
    }

    #[cfg(not(feature = "gpu"))]
    {
        cpu_cascade_running_sum_log_derivative(main_table, challenges)
    }
}


fn cpu_cascade_running_sum_log_derivative(
    main_table: ArrayView2<BFieldElement>,
    challenges: &Challenges,
) -> Array2<XFieldElement> {
...
fn auxiliary_column_public_running_evaluation(
    ...
    #[cfg(feature = "gpu")]
    {
        gpu_public_running_evaluation(main_table, challenges)
            .expect("GPU public running evaluation failed - GPU feature enabled but GPU unavailable or errored")
    }

    #[cfg(not(feature = "gpu"))]
    {
        cpu_public_running_evaluation(main_table, challenges)
    }
}

fn cpu_public_running_evaluation(
    main_table: ArrayView2<BFieldElement>,
    challenges: &Challenges,
) -> Array2<XFieldElement> {
...
#[cfg(feature = "gpu")]
fn gpu_cascade_running_sum_log_derivative(
    main_table: ArrayView2<BFieldElement>,
    challenges: &Challenges,
    ...
    // Count non-padding rows
    let non_padding_count = main_table.rows()
        .into_iter()
        .take_while(|row| !row[MainColumn::IsPadding.main_index()].is_one())
        .count();

    if non_padding_count == 0 {
        // All padding, return zeros
        return Ok(Array2::zeros((main_table.nrows(), 1)));
    }

    // Prepare compressed rows and multiplicities
    let mut compressed_rows = Vec::with_capacity(non_padding_count);
    let mut multiplicities = Vec::with_capacity(non_padding_count);

    for row in main_table.rows().into_iter().take(non_padding_count) {
        let lookup_input = row[MainColumn::LookIn.main_index()];
        let lookup_output = row[MainColumn::LookOut.main_index()];
        let compressed_row = lookup_input * look_in_weight + lookup_output * look_out_weight;
        compressed_rows.push(compressed_row);
        multiplicities.push(row[MainColumn::LookupMultiplicity.main_index()]);
    }

    // Call GPU kernel
    let mut output = vec![XFieldElement::zero(); non_padding_count];
    aux_extend::gpu_log_derivative(&compressed_rows, &multiplicities, &mut output, indeterminate)?;

    // Extend with padding (repeat last value)
    let last_value = output.last().copied().unwrap_or(XFieldElement::zero());
    output.resize(main_table.nrows(), last_value);

    Array2::from_shape_vec((main_table.nrows(), 1), output)
        .map_err(|e| e.into())
}

#[cfg(feature = "gpu")]
fn gpu_public_running_evaluation(
    main_table: ArrayView2<BFieldElement>,
    challenges: &Challenges,
) -> Result<Array2<XFieldElement>, Box<dyn std::error::Error>> {
    let indeterminate = challenges[ChallengeId::LookupTablePublicIndeterminate];
    ...
    // Prepare values
    let values: Vec<BFieldElement> = main_table.rows()
        .into_iter()
        .take(non_padding_count)
        .map(|row| row[MainColumn::LookOut.main_index()])
        .collect();

    // Call GPU kernel
    let mut output = vec![XFieldElement::zero(); non_padding_count];
    aux_extend::gpu_running_evaluation(&values, &mut output, indeterminate)?;
