...
#[cfg(feature = "gpu")]
use twenty_first::math::ntt::gpu_ntt::aux_extend;
...
/// Structure to hold all 16 cascade log derivatives
struct CascadeLogDerivatives {
    state_0_highest: Vec<XFieldElement>,
    state_0_mid_high: Vec<XFieldElement>,
    state_0_mid_low: Vec<XFieldElement>,
    state_0_lowest: Vec<XFieldElement>,
    state_1_highest: Vec<XFieldElement>,
    state_1_mid_high: Vec<XFieldElement>,
    state_1_mid_low: Vec<XFieldElement>,
    state_1_lowest: Vec<XFieldElement>,
    state_2_highest: Vec<XFieldElement>,
    state_2_mid_high: Vec<XFieldElement>,
    state_2_mid_low: Vec<XFieldElement>,
    state_2_lowest: Vec<XFieldElement>,
    state_3_highest: Vec<XFieldElement>,
    state_3_mid_high: Vec<XFieldElement>,
    state_3_mid_low: Vec<XFieldElement>,
    state_3_lowest: Vec<XFieldElement>,
}

#[cfg(feature = "gpu")]
fn gpu_hash_cascade_log_derivatives(
    main_table: ArrayView2<BFieldElement>,
    cascade_look_in_weight: XFieldElement,
    cascade_look_out_weight: XFieldElement,
    cascade_indeterminate: XFieldElement,
) -> Result<CascadeLogDerivatives, Box<dyn std::error::Error>> {
    use rayon::prelude::*;

    let num_rows = main_table.nrows();

    // Prepare data for all 16 limb pairs
    let limb_pairs = [
        (MainColumn::State0HighestLkIn, MainColumn::State0HighestLkOut),
        (MainColumn::State0MidHighLkIn, MainColumn::State0MidHighLkOut),
        (MainColumn::State0MidLowLkIn, MainColumn::State0MidLowLkOut),
        (MainColumn::State0LowestLkIn, MainColumn::State0LowestLkOut),
        (MainColumn::State1HighestLkIn, MainColumn::State1HighestLkOut),
        (MainColumn::State1MidHighLkIn, MainColumn::State1MidHighLkOut),
        (MainColumn::State1MidLowLkIn, MainColumn::State1MidLowLkOut),
        (MainColumn::State1LowestLkIn, MainColumn::State1LowestLkOut),
        (MainColumn::State2HighestLkIn, MainColumn::State2HighestLkOut),
        (MainColumn::State2MidHighLkIn, MainColumn::State2MidHighLkOut),
        (MainColumn::State2MidLowLkIn, MainColumn::State2MidLowLkOut),
        (MainColumn::State2LowestLkIn, MainColumn::State2LowestLkOut),
        (MainColumn::State3HighestLkIn, MainColumn::State3HighestLkOut),
        (MainColumn::State3MidHighLkIn, MainColumn::State3MidHighLkOut),
        (MainColumn::State3MidLowLkIn, MainColumn::State3MidLowLkOut),
        (MainColumn::State3LowestLkIn, MainColumn::State3LowestLkOut),
    ];

    // Build a map of which rows contribute (same for all limb pairs)
    let mut contributing_indices = Vec::new();
    let mut last_contributing_value = LookupArg::default_initial();
    let mut full_outputs = vec![vec![last_contributing_value; num_rows]; 16];

    for (row_idx, row) in main_table.rows().into_iter().enumerate() {
        ...
        if !in_pad_mode && !in_last_round && !current_instruction_is_sponge_init {
            contributing_indices.push(row_idx);
        }
    }

    let num_contributing = contributing_indices.len();

    if num_contributing == 0 {
        // No contributing rows, return zeros
        return Ok(CascadeLogDerivatives {
            state_0_highest: vec![XFieldElement::zero(); num_rows],
            state_0_mid_high: vec![XFieldElement::zero(); num_rows],
            state_0_mid_low: vec![XFieldElement::zero(); num_rows],
            state_0_lowest: vec![XFieldElement::zero(); num_rows],
            state_1_highest: vec![XFieldElement::zero(); num_rows],
            state_1_mid_high: vec![XFieldElement::zero(); num_rows],
            state_1_mid_low: vec![XFieldElement::zero(); num_rows],
            state_1_lowest: vec![XFieldElement::zero(); num_rows],
            state_2_highest: vec![XFieldElement::zero(); num_rows],
            state_2_mid_high: vec![XFieldElement::zero(); num_rows],
            state_2_mid_low: vec![XFieldElement::zero(); num_rows],
            state_2_lowest: vec![XFieldElement::zero(); num_rows],
            state_3_highest: vec![XFieldElement::zero(); num_rows],
            state_3_mid_high: vec![XFieldElement::zero(); num_rows],
            state_3_mid_low: vec![XFieldElement::zero(); num_rows],
            state_3_lowest: vec![XFieldElement::zero(); num_rows],
        });
    }

    // For large inputs, sequential GPU scan is too slow - fall back to CPU
    // GPU parallel scan only works correctly for ≤256 elements (single block)
    if num_rows > 256 {
        // CPU path: compute sequentially for each limb pair
        let results: Vec<Vec<XFieldElement>> = limb_pairs
            .par_iter()
            .map(|(lk_in_col, lk_out_col)| {
                let mut sum = LookupArg::default_initial();
                ...
                for row in main_table.rows() {
                    ...
                    if !in_pad_mode && !in_last_round && !current_instruction_is_sponge_init {
                        let lk_in = row[lk_in_col.main_index()];
                        let lk_out = row[lk_out_col.main_index()];
                        let compressed_row = cascade_look_in_weight * lk_in + cascade_look_out_weight * lk_out;
                        sum += (cascade_indeterminate - compressed_row).inverse();
                    }
        ...
        return Ok(CascadeLogDerivatives {
            state_0_highest: results[0].clone(),
            state_0_mid_high: results[1].clone(),
            state_0_mid_low: results[2].clone(),
            state_0_lowest: results[3].clone(),
            state_1_highest: results[4].clone(),
            state_1_mid_high: results[5].clone(),
            state_1_mid_low: results[6].clone(),
            state_1_lowest: results[7].clone(),
            state_2_highest: results[8].clone(),
            state_2_mid_high: results[9].clone(),
            state_2_mid_low: results[10].clone(),
            state_2_lowest: results[11].clone(),
            state_3_highest: results[12].clone(),
            state_3_mid_high: results[13].clone(),
            state_3_mid_low: results[14].clone(),
            state_3_lowest: results[15].clone(),
        });
    }

    // GPU path for small inputs (≤256 rows)
    let results: Vec<Vec<XFieldElement>> = limb_pairs
        .par_iter()
        .enumerate()
        .map(|(limb_idx, (lk_in_col, lk_out_col))| {
            // Build full arrays with zeros for non-contributing rows
            let mut compressed_rows = Vec::with_capacity(num_rows);
            let mut multiplicities = Vec::with_capacity(num_rows);
            ...
            for row in main_table.rows() {
                ...
                if !in_pad_mode && !in_last_round && !current_instruction_is_sponge_init {
                    ...
                    let lk_out = row[lk_out_col.main_index()];
                    let compressed_row = cascade_look_in_weight * lk_in + cascade_look_out_weight * lk_out;
                    compressed_rows.push(compressed_row);
                    multiplicities.push(BFieldElement::one());
                } else {
                    // For non-contributing rows: push dummy value with zero multiplicity
                    compressed_rows.push(XFieldElement::zero());
                    multiplicities.push(BFieldElement::zero());
                }
            }

            // Call GPU kernel with full arrays
            let mut output = vec![XFieldElement::zero(); num_rows];
            aux_extend::gpu_log_derivative(&compressed_rows, &multiplicities, &mut output, cascade_indeterminate)
                .expect("GPU log derivative failed");

            output
        })
        .collect();

    Ok(CascadeLogDerivatives {
        state_0_highest: results[0].clone(),
        state_0_mid_high: results[1].clone(),
        state_0_mid_low: results[2].clone(),
        state_0_lowest: results[3].clone(),
...
impl TraceTable for HashTable {
    ...
    fn extend(
        ...
        // Compute all 16 cascade log derivatives on GPU
        #[cfg(feature = "gpu")]
        let gpu_cascade_log_derivs = gpu_hash_cascade_log_derivatives(
            main_table,
            cascade_look_in_weight,
            cascade_look_out_weight,
            cascade_indeterminate,
        ).expect("GPU hash cascade log derivatives failed");
        ...
        for row_idx in 0..main_table.nrows() {
            ...
            // CPU: accumulate log derivatives
            #[cfg(not(feature = "gpu"))]
            if !in_pad_mode && !in_last_round && !current_instruction_is_sponge_init {
