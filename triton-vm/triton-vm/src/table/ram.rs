...
impl TraceTable for RamTable {
    ...
    fn fill(
        ...
        profiler!(start "bezout_coefficient_polynomials_coefficients");
        let (bezout_0, bezout_1) =
            bezout_coefficient_polynomials_coefficients(&unique_ram_pointers);
        profiler!(stop "bezout_coefficient_polynomials_coefficients");

        profiler!(start "make_ram_table_consistent");
        let ret = make_ram_table_consistent(&mut ram_table, bezout_0, bezout_1);
        profiler!(stop "make_ram_table_consistent");
...
pub fn bezout_coefficient_polynomials_coefficients(
    ...
    profiler!(start "par_zerofier");
    let rp = Polynomial::par_zerofier(unique_roots);
    profiler!(stop "par_zerofier");
    
    profiler!(start "formal_derivative");
    let fd = rp.formal_derivative();
    profiler!(stop "formal_derivative");

    profiler!(start "par_batch_evaluate");
    let fd_in_roots = fd.par_batch_evaluate(unique_roots);
    profiler!(stop "par_batch_evaluate");

    profiler!(start "batch_inversion");
    let b_in_roots = BFieldElement::batch_inversion(fd_in_roots);
    profiler!(stop "batch_inversion");

    profiler!(start "b par_interpolate");
    let b = Polynomial::par_interpolate(unique_roots, &b_in_roots);
    profiler!(stop "b par_interpolate");

    profiler!(start "a + one_minus_fd_b divide");
    let one_minus_fd_b = Polynomial::one() - fd.multiply(&b);
    let a = one_minus_fd_b.clean_divide(rp);
    profiler!(stop "a + one_minus_fd_b divide");

    profiler!(start "unique_roots resize");
    ...
    profiler!(stop "unique_roots resize");
