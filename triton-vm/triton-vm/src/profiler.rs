...
/// Execution mode for a profiled task
...
impl ExecutionMode {
    fn suffix(self) -> &'static str {
        match self {
            ...
            ExecutionMode::MultiGpu => " (multi-gpu)",
        }
    }
}
...
struct Task {
    ...
    /// The execution mode (CPU, GPU, or multi-GPU)
...
impl Display for VMPerformanceProfile {
    fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
        ...
        for task in &self.tasks {
            ...
            let task_name_with_mode = format!("{}{}", task.name, task.execution_mode.suffix());
            let task_name = task_name_with_mode.color(task.weight.color());
...
/// use crate::profiler::ExecutionMode;
/// crate::profiler::start("heavy lifting");
/// profiler!(start "good job" ("compute")); // uses the category "compute", CPU mode
/// profiler!(start "gpu work" ("compute") [gpu]); // GPU mode
/// profiler!(start "multi work" ("compute") [multi-gpu]); // multi-GPU mode
/// for _ in 0..5 {
///     profiler!(start "loop"); // category and mode are optional (defaults to CPU)
///     /* work hard */
///     profiler!(stop "loop"); // must match exactly
/// }
/// profiler!(stop "good job"); // category and mode are inferred when stopping
/// let profile = crate::profiler::finish();
/// ```
macro_rules! profiler {
    (start $task:literal ($category:literal) [gpu]) => {{
        #[cfg(any(debug_assertions, not(feature = "no_profile")))]
        $crate::profiler::PROFILER.with_borrow_mut(|profiler| {
            if let Some(profiler) = profiler.as_mut() {
                let here = $crate::profiler::here!();
                profiler.start($task, here, Some($category.to_string()), $crate::profiler::ExecutionMode::Gpu);
            }
        })
    }};
    (start $task:literal ($category:literal) [multi-gpu]) => {{
        #[cfg(any(debug_assertions, not(feature = "no_profile")))]
        $crate::profiler::PROFILER.with_borrow_mut(|profiler| {
            if let Some(profiler) = profiler.as_mut() {
                let here = $crate::profiler::here!();
                profiler.start($task, here, Some($category.to_string()), $crate::profiler::ExecutionMode::MultiGpu);
            }
        })
    }};
    (start $task:literal ($category:literal)) => {{
        ...
        $crate::profiler::PROFILER.with_borrow_mut(|profiler| {
            if let Some(profiler) = profiler.as_mut() {
                let here = $crate::profiler::here!();
                profiler.start($task, here, Some($category.to_string()), $crate::profiler::ExecutionMode::Cpu);
            }
        })
    }};
    (start $task:literal [gpu]) => {{
        #[cfg(any(debug_assertions, not(feature = "no_profile")))]
        $crate::profiler::PROFILER.with_borrow_mut(|profiler| {
            if let Some(profiler) = profiler.as_mut() {
                profiler.start($task, $crate::profiler::here!(), None, $crate::profiler::ExecutionMode::Gpu);
            }
        })
    }};
    (start $task:literal [multi-gpu]) => {{
        #[cfg(any(debug_assertions, not(feature = "no_profile")))]
        $crate::profiler::PROFILER.with_borrow_mut(|profiler| {
            if let Some(profiler) = profiler.as_mut() {
                profiler.start($task, $crate::profiler::here!(), None, $crate::profiler::ExecutionMode::MultiGpu);
            }
        })
    }};
    (start $task:literal) => {{
        ...
        $crate::profiler::PROFILER.with_borrow_mut(|profiler| {
            if let Some(profiler) = profiler.as_mut() {
                profiler.start($task, $crate::profiler::here!(), None, $crate::profiler::ExecutionMode::Cpu);
            }
        })
    }};
    (stop $task:literal) => {{
