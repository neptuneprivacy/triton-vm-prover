...
pub(crate) async fn create_block_transaction_from(
    ...
    // Note: NOP proof was already generated in parallel with coinbase, so we can use it immediately
    if transactions_to_merge.is_empty() {
        info!("Using pre-generated nop transaction to set merge bit through a merge");
