...
impl<P: Ord + Send + Sync + 'static> JobQueue<P> {
    ...
    pub fn add_job(
        ...
        let (num_jobs, job_running) = {
            ...
            let job_running = if guard.running_jobs.is_empty() {
                "none".to_string()
            } else {
                guard.running_jobs.iter()
                    .map(|j| format!("#{} - {}", j.job_num, j.job_id))
                    .collect::<Vec<_>>()
                    .join(", ")
            };
        ...
        tracing::info!(
            "JobQueue: job added - {}  {} queued job(s).  job running: {} (max_concurrent: {})",
            job_id,
            num_jobs,
            job_running,
            self.shared_queue.lock().unwrap().max_concurrent_jobs
        );
    ...
    pub fn num_jobs(&self) -> usize {
        ...
        guard.jobs.len() + guard.running_jobs.len()
    }
...
///
/// Supports concurrent execution of multiple jobs when max_concurrent_jobs > 1.
/// This enables parallel proof generation for GPU-bound proofs.
///
/// The function maintains a set of running jobs and starts new jobs when:
/// 1. A job is added to the queue
/// 2. A running job completes and there are queued jobs
/// 3. The number of running jobs is below max_concurrent_jobs
async fn process_jobs<P: Ord + Send + Sync + 'static>(
    ...
    let (tx_completion, mut rx_completion) = mpsc::unbounded_channel::<(JobId, JobCompletion)>();
    
    // job number starts at 1 and increments with each job that is processed.
    let mut job_num: usize = 1;
    
    // Track running jobs: job_id -> (job_info, result_tx)
    let mut running_jobs: HashMap<JobId, (CurrentJob, oneshot::Sender<JobCompletion>)> = HashMap::new();
    
    // Helper function to start a new job if we're under the limit
    let try_start_job = |shared_queue: &Arc<Mutex<SharedQueue<P>>>, 
                         job_num: &mut usize,
                         tx_completion: &mpsc::UnboundedSender<(JobId, JobCompletion)>,
                         running_jobs: &mut HashMap<JobId, (CurrentJob, oneshot::Sender<JobCompletion>)>| -> bool {
        let mut guard = shared_queue.lock().unwrap();
        
        // Check if we can start more jobs
        if guard.running_jobs.len() >= guard.max_concurrent_jobs {
            return false;
        }
        ...
        guard.jobs.make_contiguous().sort_by(|a, b| b.priority.cmp(&a.priority));
        let job = guard.jobs.pop_front().unwrap();
        
        let job_id = job.job_id;
        let result_tx = job.result_tx;
        let cancel_rx = job.cancel_rx;
        let cancel_tx = job.cancel_tx.clone();
        ...
        guard.running_jobs.push(current_job.clone());
        let num_pending = guard.jobs.len();
        let num_running = guard.running_jobs.len();
        drop(guard);
        
        tracing::info!(
            "  *** JobQueue: begin job #{} - {} - {} queued job(s) - {} running (max: {}) ***",
            *job_num,
            job_id,
            num_pending,
            num_running,
            shared_queue.lock().unwrap().max_concurrent_jobs
        );
        
        // Spawn task that performs the job and sends completion notification
        let tx_completion = tx_completion.clone();
        if job.job.is_async() {
            tokio::spawn(async move {
                let completion = job.job.run_async_cancellable(cancel_rx).await;
                let _ = tx_completion.send((job_id, completion));
            });
        } else {
            let job_box = job.job;
            tokio::spawn(async move {
                let completion = tokio::task::spawn_blocking(move || job_box.run(cancel_rx)).await
                    .unwrap_or_else(|e| {
                        if e.is_panic() {
        ...
        running_jobs.insert(job_id, (current_job, result_tx));
        *job_num += 1;
        true
    };
    ...
    loop {
        ...
        while try_start_job(&shared_queue, &mut job_num, &tx_completion, &mut running_jobs) {
            // Keep starting jobs until we're at capacity or queue is empty
        }
        
        if running_jobs.is_empty() {
            // No jobs running, wait for a new job to be added or stop signal
            tokio::select! {
                _ = rx_job_added.recv() => {
                    // New job added, continue loop to start it
                    continue;
                }
                _ = rx_stop.changed() => {
                    handle_stop_signal_concurrent(&shared_queue, &running_jobs).await;
                    break;
                }
            }
        } else {
            // Wait for any running job to complete, or new job to be added, or stop signal
            tokio::select! {
                result = rx_completion.recv() => {
                    if let Some((completed_job_id, job_completion)) = result {
                        // A job completed
                        let (current_job, result_tx) = running_jobs.remove(&completed_job_id)
                            .expect("Completed job not in running_jobs");
                        
                        // Remove from running jobs in shared queue
                        {
                            let mut guard = shared_queue.lock().unwrap();
                            guard.running_jobs.retain(|j| j.job_id != completed_job_id);
                        }
                        
                        tracing::debug!(
                            "  *** JobQueue: ended job #{} - {} - Completion: {} ***",
                            current_job.job_num,
                            current_job.job_id,
                            job_completion
                        );
                        
                        // Send result to JobHandle
                        if let Err(e) = result_tx.send(job_completion) {
                            tracing::warn!("job-handle dropped? {}", e);
                        }
                        
                        // Try to start a new job now that we have capacity
                        // This is critical for parallel execution - we immediately check for queued jobs
                        continue;
                    }
                }
                _ = rx_job_added.recv() => {
                    // New job added, try to start it immediately if we have capacity
                    // This ensures concurrent execution when max_concurrent_jobs > 1
                    continue;
                }
                _ = rx_stop.changed() => {
                    handle_stop_signal_concurrent(&shared_queue, &running_jobs).await;
...
/// handles the 'stop' branch of tokio::select!{} in process_job() task (concurrent version)
async fn handle_stop_signal_concurrent<P: Ord + Send + Sync + 'static>(
    _shared_queue: &Arc<Mutex<SharedQueue<P>>>,
    running_jobs: &std::collections::HashMap<JobId, (CurrentJob, oneshot::Sender<JobCompletion>)>,
) {
    tracing::debug!("task process_jobs received Stop message.");

    // Cancel all running jobs - use the running_jobs map to get cancel_tx
    let cancel_txs: Vec<_> = running_jobs.iter()
        .map(|(job_id, (current_job, _))| (*job_id, current_job.cancel_tx.clone()))
        .collect();
    
    for (job_id, cancel_tx) in cancel_txs {
        match cancel_tx.send(()) {
            Ok(()) => {
                tracing::debug!(
                    "JobQueue: notified job {} to cancel.  waiting...",
                    job_id
                );
                cancel_tx.closed().await;
                tracing::debug!("JobQueue: job {} has cancelled.", job_id);
            }
            Err(e) => {
                tracing::warn!(
                    "could not send cancellation msg to job {}. {}",
                    job_id,
                    e
                )
            }
